...
DBHOST=$(oc get svc db2-lb -n b2bi-prod -o jsonpath='{ .status.loadBalancer.ingress[0].ip}')
DBPORT=$(oc get svc db2-lb -n b2bi-prod -o jsonpath='{ .spec.ports[0].port}')
DBDATA=${DBDATA:-"B2BIDB"}
DBCREATESCHEMA=${DBCREATESCHEMA:-"true"}
JMSHOST=$(oc get svc mq-data -n b2bi-prod -o jsonpath='{ .spec.clusterIP}')
JMSPORT=$(oc get svc mq-data -n b2bi-prod -o jsonpath='{ .spec.ports[0].port}')
